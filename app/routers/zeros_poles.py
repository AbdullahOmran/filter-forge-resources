...
def create_zeros_poles(workspace_id: int, zeros_poles: schemas.ZerosPolesCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def read_zeros_poles(workspace_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    workspace = crud.get_workspace(db=db, workspace_id=workspace_id)
    if workspace is None or workspace.user_id != current_user.id:
        raise HTTPException(status_code=404, detail="Workspace not found")
...
def delete_zeros_poles(zeros_poles_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
